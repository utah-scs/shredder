...
//                to_string(k).copy(key, 8);
 ...
 //           printf("before %u %lu\n", engine().cpu_id(), _client_id);
            if (_ycsb_client->workload == "YCSB-SCAN") {
                ...
                command = build_redis_command({"GET", key});
                //command = build_redis_command({"JS", "get", key});
            }
            else {
            ...
            *s = steady_clock_type::now();
            //return _write_buf.write(command).then([this, s] {

            // Batching 8 requests and then wait for replies
...
*/
    ...
    future<> connect(ipv4_addr server_addr) {
        ...
        for (unsigned i = 0; i < _conn_per_core; i++) {
            //usleep(100);
            auto source = "0.0.0.0:112" + to_string(engine().cpu_id()) + to_string(i);
            cout << source << "\n";
            engine().net().connect(make_ipv4_address(server_addr), make_ipv4_address(ipv4_addr{source})).then([this] (connected_socket fd) {
