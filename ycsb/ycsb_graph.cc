...
//                to_string(k).copy(key, 8);
 ...
 //           printf("before %u %lu\n", engine().cpu_id(), _client_id);
            if (_ycsb_client->workload == "YCSB-SCAN") {
                ...
                for (int i = 0; i < 8; i++) {
                    memset(key, 0, keyLen);
                    //auto k = generateRandom()%4031;
                    auto k = keys[generateRandom()%keys.size()];
                    to_string(k).copy(key, 8);
                ...
                if (_ycsb_client->friends.empty()) {
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        to_string(k).copy(key, 8);
                        command = command + build_redis_command({"GET", key});
                        _ycsb_client->friends.insert(_ycsb_client->friends.end(), graph[k].begin(), graph[k].end());
                        _ycsb_client->num++;
                        len += graph[k].size();
                        if (graph[k].size() == 0)
                            len += 1;
                    ...
                    for (int i = 0; i < 8; i++) {
                        memset(key, 0, keyLen);
                        auto k = _ycsb_client->friends[0];
                        to_string(k).copy(key, 8);
                        command = command + build_redis_command({"GET", key});
                        len += graph[k].size();
                        if (graph.find(k) == graph.end()) len++;
                        _ycsb_client->friends.erase(_ycsb_client->friends.begin());
                        if (len > 200)
                           break;
                        if (_ycsb_client->friends.empty()) {
                            _nr_done += _ycsb_client->num;
                            break;
                        }
                    }
                }
        ...
        std::chrono::steady_clock::time_point* s = new std::chrono::steady_clock::time_point();
            ...
            return _write_buf.write(command).then([this, s, command, len] {
            return _write_buf.flush().then([this, s, len] {
                return _read_buf.read_up_to(len*8).then([this, s] (temporary_buffer<char> buf) {
//                    printf("%s\n", buf.get());
