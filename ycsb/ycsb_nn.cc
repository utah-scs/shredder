...
//vector<float> input{0.08131241084165478, 0.5263157894736841, 0.031620553359683806, 0.18716577540106946, 0.2783505154639176, 0.17391304347826086, 0.33448275862068966, 0.3565400843881856, 0.2075471698113207, 0.3312302839116719, 0.28327645051194533, 0.5772357723577236, 0.4432234432234432};
//vector<float> input{0.1666666666666668, 0.41666666666666663, 0.06779661016949151, 0.04166666666666667};
unsigned num_inputs = 13;
unsigned num_hidden_weights = num_inputs + 1;
unsigned num_hiddens = 5;
unsigned num_outputs = 3;
unsigned num_output_weights = num_hiddens + 1;

/*
unsigned num_inputs = 4;
unsigned num_hidden_weights = num_inputs + 1;
unsigned num_hiddens = 2;
unsigned num_outputs = 3;
unsigned num_output_weights = num_hiddens + 1;
*/
unsigned predict(float* nn, float* input) {
    vector<float> old_input;
    for (unsigned i = 0; i < num_inputs; i++) {
        old_input.push_back(*(input+i));
    }

    vector<float> new_input;
    for (unsigned i = 0; i < num_hiddens; i++) {
        auto index = num_hidden_weights*i;
        auto activation = nn[index + num_hidden_weights -1];
        for (unsigned j = 0; j < num_hidden_weights - 1; j++) {
            activation += nn[index + j]*old_input[j];
        }
        new_input.push_back(1.0/(1.0 + exp(-1*activation)));
    }
    old_input = new_input;
    new_input.clear();

    for (unsigned i = 0; i < num_outputs; i++) {
        auto index = num_hidden_weights*num_hiddens + i*num_output_weights;
        auto activation = nn[index + num_output_weights -1];
        for (unsigned j = 0; j < num_output_weights - 1; j++) {
            activation += nn[index + j]*old_input[j];
        }
        new_input.push_back(1.0/(1.0 + exp(-1*activation)));
    }
    old_input = new_input;
    return max_element(old_input.begin(),old_input.end()) - old_input.begin();
}
 ...
 //           printf("before %u %lu\n", engine().cpu_id(), _client_id);
            if (_ycsb_client->workload == "YCSB-SCAN") {
                ...
                for (int i = 0; i < 16; i++) {
                    ...
                    //command = command + build_redis_command({"JS", "get_friend_list", key});
            ...
            *s = steady_clock_type::now();
            ...
            return _write_buf.flush().then([this, s] {
                return _read_buf.read_exactly((352+13*4)*8).then([this, s] (temporary_buffer<char> buf) {
                //return _read_buf.read_exactly((76+16)*8).then([this, s] (temporary_buffer<char> buf) {
                //return _read_buf.read().then([this, s] (temporary_buffer<char> buf) {
                    //printf("%s\n", buf.get());
                    /*auto arr = (float*)buf.get();
                    for (int i = 0; i < 8; i++) {
                        predict(arr, arr + 19);
//cout << res << "\n";
                        arr += (19+4);
...
int main(int ac, char** av) {
    ...
    app.add_options()
        ...
        ("workload,w", bpo::value<std::string>()->default_value("YCSB-A"), "Type of YCSB workload")
